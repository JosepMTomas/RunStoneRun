...
public class SmokeParticleSystem
{
	...
	private void emitParticle(float currentPositionZ, float[] currentDirection)
	{
		...
		if(isEnabled && numberOfParticles < MAX_NUMBER_OF_PARTICLES)
		{
			// Increment the index
			currentIndex++;
			...
			// New rotation
			if(angle < 0)
				rotations[currentIndex] = random.nextFloat() * 1f;
			else
				rotations[currentIndex] = random.nextFloat() * -1f;
	...
	public void update(float[] viewProjection, float currentPositionZ, float[] currentDirection, float[] displacement, float currentSpeed, float deltaTime)
	{
		...
		if(isEnabled)
		{
			...
			for(int i=0; i<MAX_NUMBER_OF_PARTICLES; i++)
			{
				...
				angles[i] = angles[i] + rotations[i];

				if(timers[i] > PARTICLE_MAXIMUM_LIFE)
				{
					timers[i] = 0f;
					numberOfParticles--;
				}
	...
	public void draw()
	{
		//Log.d(TAG, "Number of particles: " + numberOfParticles);
